...
- **Stanley Cup winner derivation assumes the modern 8-4-2-1 bracket.** `standings.cup_winner(season)` chronologically buckets playoff series into Round 1 (first 8), Round 2 (next 4), Conference Finals (next 2), and SCF (last 1). This works for every season since 2013 and most seasons back to the late 1960s. It breaks for the **2019-20 (COVID bubble, 24-team play-in)** and **2020-21 (realigned all-divisional brackets)** seasons, where the series count doesn't fit the 8-4-2-1 shape. For those two seasons, a hand-curated `CUP_WINNER_OVERRIDES` dict in `standings.py` carries the ground truth (TBL won both). For very early seasons (pre-1942) when the Cup was sometimes decided via inter-league challenge series, the derived winner reflects the NHL playoff winner, which may differ from the actual Cup holder. Affects only the dashboard's 👑 marker — the rating model is unaffected.
...
- **Dark "broadcast graphics" theme with NHL accent red.** The dashboard uses a charcoal background (`--bg: #0d0e10`) with NHL red (`#c8102e`) as the accent, deliberately echoing NHL broadcast lower-thirds. This choice forces a trade-off on team colors whose official primary is black (BOS, PIT, LAK): on dark backgrounds those lines would be invisible, so the codebase substitutes their published secondary (BOS gold `#FFB81C`, PIT Vegas gold `#FCB514`, LAK silver `#A2AAAD`). A few teams with very dark blue/green primaries (COL burgundy, DAL green, FLA / WPG navy, STL / TBL / TOR / VAN blue, MIN green) also have slightly lightened variants so they read against the dark UI. Anyone restoring the published primaries needs to also restore the light theme — the two are coupled.
